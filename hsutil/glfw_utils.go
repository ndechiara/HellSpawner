...
func (v Texture) GetHandle() uint32 {
	return v.handle
}
...
func (v *Texture) Bind(texUnit uint32) {
	gl.ActiveTexture(texUnit)
	gl.BindTexture(v.target, v.handle)
	v.texUnit = texUnit
}

func (v *Texture) UnBind() {
	v.texUnit = 0
	gl.BindTexture(v.target, 0)
}

func (v *Texture) SetUniform(uniformLoc int32) error {
	if v.texUnit == 0 {
		return errTextureNotBound
	}
	gl.Uniform1i(uniformLoc, int32(v.texUnit-gl.TEXTURE0))
	return nil
}
